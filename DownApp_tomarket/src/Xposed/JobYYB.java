	...
	public void handleAftreMethod(String packageName,MethodHookParam param, int type) {
		...
		case MethodInt.VIEW_TOSTRING_YYB_TV_LOAD:
			...
			if (obj instanceof TextView) {//2131231373
				...
				if ("暂不开启".equals(t.getText().toString()) &&t.getWidth() > 0) {
					boolean res=SetClickUtil.setClick(t);
					Logger.i("YYB_LOAD-------yyb ---暂不开启-------"+t.getId()+"===="+t.getVisibility()+"  res:"+res);

				}
				if (((t.getId() == 2131559032|| (t.getId() ==2131231373)))&& t.getWidth() > 0 && !yybGo) {
					Logger.i("YYB_LOAD-------yyb ------VIEW_TOSTRING_YYB_TV_LOAD-------"+t.getId()+"===="+t.getVisibility());
					yybGo = true;
				}
			}// 没有break
//			---------disp-----------------2131559424=====暂不开启===409----class
		case MethodInt.VIEW_TOSTRING_YYB_DOWN:
			...
			if (obj instanceof View) {
				...
				// 快速安装2131231818
				if (((view.getId() == 2131559413)||(view.getId()==2131231818)||view.getId()==2131559424) && view.getWidth() > 0) {
					if (yybLiuliang)
						return;
					yybLiuliang = false;
					Message msg = handler.obtainMessage();
