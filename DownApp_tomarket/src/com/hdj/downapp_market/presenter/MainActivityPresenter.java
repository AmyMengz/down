...
public class MainActivityPresenter {
	...
	public void init() {
		...
		mainActivityView.setAutoAccountChecked();
		getAutoAccountEanble();
		mainActivityView.setTitles(String.valueOf(SprefUtil.getInt(context,
				SprefUtil.C_COUNT, 0)));
		initRebootTime();
	...
	public void isGooglePlayAccountFileAccessible() {
		if (SprefUtil.getInt(context, SprefUtil.C_MARKET,
			...
			if (MyFileUtil.getGoogleAccountCount() == 0
					|| MyFileUtil.getAccountIndex() >= MyFileUtil
							.getGoogleAccountCount()) {
				mainActivityView.toast(context
						.getString(R.string.google_market_no_more_account));// googel
																			// play
																			// 没有可用账户
				AlertDialog dialog = new AlertDialog.Builder(context)
						.setTitle(
								context.getString(R.string.please_add_account_file))
	...
	private void getVoicePermissionPrepare() {
		new Thread(new Runnable() {
			...
			@Override
			public void run() {
				MyFileUtil.copySoundFile(context,
						GlobalConstants.SOUND_ROOT_NAME,
						GlobalConstants.SOUND_ROOT_NAME);
				MyFileUtil.copySoundFile(context,
						GlobalConstants.SOUND_GOOGLE_NAME,
						GlobalConstants.SOUND_GOOGLE_NAME);
	...
	public void openChoseMarket() {
		((MainActivity) context).startActivityForResult(new Intent(context,
				ChoseMarketActivity.class), GlobalConstants.requestMarket);
	}
	...
	public void getTvChooseMarket() {
		MARKET market;
		int value = SprefUtil.getInt(context, SprefUtil.C_MARKET,
				MARKET.MARKET_BAIDU.value());
		...
		mainActivityView.setTvChooseMarket(String.format(
				context.getString(R.string.choose_market_tips2), marketTips));
		if (SprefUtil.getBool(context, SprefUtil.C_GOOGLE_VPN, false)) {
			if (market.value() == MARKET.MARKET_GP.value()) {
				EasyOperateClickUtil.setAutoVpnFlag(context,
						EasyOperateClickUtil.AUTO_VPN);
			} else {
				EasyOperateClickUtil.setAutoVpnFlag(context,
						EasyOperateClickUtil.DO_NOT_AUTO_VPN);
			}
		} else {
			EasyOperateClickUtil.setAutoVpnFlag(context,
					EasyOperateClickUtil.DO_NOT_AUTO_VPN);
	...
	/**
	 * 如果当前选中的是Google Play 可选
	 */
	private void getAutoAccountEanble() {
		boolean enable = SprefUtil.getInt(context, SprefUtil.C_MARKET,
				MARKET.MARKET_BAIDU.value()) == MARKET.MARKET_GP.value();
		Logger.i("----------"+enable);
		mainActivityView.setTvAutoAccountEanble(enable);
		mainActivityView.setAutoAccountEnable(enable);
	}
	...
	public void openChoseAPK() {
		((MainActivity) context).startActivityForResult(new Intent(context,
				ChoseDownAppActivity.class), GlobalConstants.requestAPK);
	}
	...
	public void getTvApkTips() {
		...
		if (list.size() <= 0 || selection < 0) {
			mainActivityView.setTvChoseApk(context
					.getString(R.string.choose_to_add_apk));
		} else {
			if (selection >= list.size()) {
				selection = list.size() - 1;
				SprefUtil.putInt(context, SprefUtil.C_SELECTION, selection);
			}
			mainActivityView.setTvChoseApk(String.format(
					context.getString(R.string.current_pn_tips),
					list.get(selection).getPackageName()));
		}
	}
	...
	public void getTvDownPercentTips() {
		...
		if (MARKET.valueOf(value) == MARKET.MARKET_WDJ) {
			mainActivityView.setTvDownPercentTips(context
					.getString(R.string.wdj_no_half));

		} else {
			int percent = EasyOperateClickUtil.getHalfDownloadValue(context);
			mainActivityView.setTvDownPercentTips(String.format(
					context.getString(R.string.current_percent_tips), percent));
		}
	}
	...
	public void jumpMarket() {
		...
		case MARKET_GP:
			...
			// public void run() {
			// while
			// (EasyOperateClickUtil.getIsGotoGoogleStoreFlag(context)!=EasyOperateClickUtil.GOTO_GOOGLE_STORE)
			// {
			// try {
			// Thread.sleep(1000);
			// } catch (InterruptedException e) {
			...
			// }
			// Logger.e("---￥￥￥￥￥￥￥￥￥￥￥￥4-----MARKET_GP---------MARKET_GP----------MARKET_GP----￥￥￥￥￥￥￥￥￥￥￥￥￥￥----"+EasyOperateClickUtil.getIsGotoGoogleStoreFlag(context));
			// EasyOperateClickUtil.setIsotoGoogleStoreFlag(context,
			// EasyOperateClickUtil.DO_NOT_GOTO_GOOGLE_STORE);
			// // gpThread=false;
			// Thread.interrupted();
			// openMarket(context, GlobalConstants.GP_PN, url);
			// }
	...
	public void tbAutoAccountInject(boolean isChecked) {
		EasyOperateClickUtil.setIsAutoInjectAccountFlag(context,
				isChecked ? EasyOperateClickUtil.AUTO_INJECT_ACCOUNT
						: EasyOperateClickUtil.DO_NOT_AUTO_INJECT_ACCOUNT);
	}
	...
	public void initRebootTime() {
		...
		if (rebootCount > 0) {
			mainActivityView
					.setRebootTime(String.format(
							context.getString(R.string.reboot_recode_tips),
							rebootCount));
		} else {
			mainActivityView.setRebootTime(context
					.getString(R.string.reboot_recode_tips_no_reboot));
		}
	...
	public void clearCount() {
		...
		initRebootTime();
		mainActivityView.setTitles(SprefUtil.getInt(context, SprefUtil.C_COUNT,
				0) + "");
	}
	...
	public void ttsRootTest() {
		...
		in.putExtra(GlobalConstants.ALERT_ACTIVITY_FLAG,
				GlobalConstants.ALERT_ACTIVITY_NO_ROOT);
	...
	public void ttsGoogelTest() {
		...
		Intent in = new Intent(context, AlertActivity.class);
		in.putExtra(GlobalConstants.ALERT_ACTIVITY_FLAG,
				GlobalConstants.ALERT_ACTIVITY_NO_ACCOUNT);
		in.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
		context.startActivity(in);
	}

	public void chooseGoogleAccountSet() {
		mainActivityView.setAutoAccountChecked();
		getAutoAccountEanble();
	}
